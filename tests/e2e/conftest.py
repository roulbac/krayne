...
Self-contained (rather than shared via ``tests/conftest.py``) on purpose: the
session manager-reaper below SIGKILLs processes recorded under the *real*
``~/.krayne/tunnels`` — ``tunnel_state.TUNNEL_DIR`` is bound at import and the
unit suite does not redirect it. Scoping these fixtures to the e2e package keeps
that teardown away from unit/integration sessions.
"""
...
def sandbox_kubeconfig() -> str:
    """Tear down any existing sandbox, set up a fresh one, tear down after."""
...
def _reap_leaked_managers():
    """Best-effort SIGKILL of any manager subprocesses left after the run."""
    yield
